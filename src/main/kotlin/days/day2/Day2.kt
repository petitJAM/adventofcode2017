...
//                listOf(3, 8, 6, 5))
//
//        println(solve2(testInput2))
//        println("Expected: 9")
    }

    private val input: List<List<Int>>
        get() {
            val raw = "1208\t412\t743\t57\t1097\t53\t71\t1029\t719\t133\t258\t69\t1104\t373\t367\t365\n" +
    ...
    fun solve2(input: List<List<Int>>): Int {
        return input
                ...
                .map { row ->
                    findDivision(row.first(), row.drop(1))
                }
                .sum()
    }

    private tailrec fun findDivision(head: Int, tail: List<Int>): Int {
        return tail
                .firstOrNull {
                    head % it == 0
                }
                ?.let { head / it }
                ?: findDivision(tail.first(), tail.drop(1))
    }
}